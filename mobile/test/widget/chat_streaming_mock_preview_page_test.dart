...
void main() {
  testWidgets(
    '发送消息后直播态块序列依次出现(工具进度+A2UI 卡片),'
    '流式结束后交接为持久化历史,内容与直播时一致',
    (tester) async {
      ...
      // 直接驱动真实发送流程背后的 [ChatSend] notifier,聚焦验证块序列直播
      // 渲染管线本身。
      ...
      // 逐帧推进,直播剧本按顺序吐出:文本 → 工具调用中/已完成 → 文本 → A2UI 卡片。
      // 用标志位记录整个直播过程中有没有出现过这些过渡态,而不是卡死在某一帧断言,
      // 因为过渡态只在某几帧短暂存在。
      ...
      expect(tester.takeException(), isNull);
      expect(sawToolCalling, isTrue, reason: '直播过程中应该出现过"正在调用"过渡提示');
      expect(sawToolDone, isTrue, reason: '直播过程中应该出现过"已完成"过渡提示');
      expect(sawLiveSurface, isTrue, reason: '直播过程中应该出现过 A2UI 卡片(直播临时的)');

      // 再多推进几帧,确保 ChatDoneEvent 已处理、syncMessages 已把最终消息落库、
      // 状态切回 idle,直播占位气泡消失、变成持久化历史消息。
