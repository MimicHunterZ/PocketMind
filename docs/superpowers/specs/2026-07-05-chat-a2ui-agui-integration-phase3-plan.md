...
## 卡片交互提交往返(D15)— ✅ 已补做(2026-07-08,同轮追加)

原计划留到下一轮,用户要求一并做。落地发现比原设想更省:**不需要给后端加 `dataModel` 字段**——
提交复用现有 `send()`,content 存 `{surfaceId, dataModel}` JSON,后端按普通 USER 消息零改动落库
并喂回模型(D15 早已定这个形状)。三处改动:

- **后端选项 value 改为可读标题(A2uiChoiceCardToolSet)**:原来是 `option-N` 索引,用户选完提交回来
  模型只看到 `{"selected":["option-2"]}` 读不懂,往返失去意义。改成用 `item.title()` 作 value,让提交的
  dataModel 对模型可读(D7)。
- **前端接线提交(ChatMessageBubble)**:`a2uiCardSubmitHandlerProvider` 注入的 handler 优先(debug 预览页
  写本地假仓库);生产为 null 时退回 `chatSend.send(jsonEncode({surfaceId,dataModel}), showPendingBubble:false)`,
  把提交当作一次新用户消息发出,自然落库 + reload 据它推导锁定态。
- **提交消息不渲染成气泡(chat_list_formatter)**:提交消息是 `{surfaceId,dataModel}` JSON 的 USER 消息,
  只驱动锁定态和喂模型历史,不作为气泡(卡片已定格显示选择)。收集其 uuid,渲染时跳过,并保证它不抢占
  `isLastUserMsg`/`isLastOfTurn`。

测试:formatter 新增"提交消息隐藏 + 不抢占 turn/编辑入口"用例;lock 预览页 widget 测试(注入 handler 路径)
仍绿;后端 16 测试全绿(A2ui 无 option-N 断言,无破坏)。**未处理**:用户填一半没点确认的纯本地中间态
reload 不恢复(spec D15 已列为已知限制,本次仍不做)。
...
- **Never**:改动后端 `toToolResultJson` 包装形状 / `parseToolResult`(D12 跨轮历史)/ `parseToolCallData`(E2 用前端剥包装规避,不动后端);为兼容旧 `delta`/`done` 事件保留死代码(D8)。
