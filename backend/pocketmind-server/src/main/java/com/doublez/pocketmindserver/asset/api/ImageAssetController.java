 ...
 * 端点：
 * <ul>
 *   <li>{@code POST /api/assets/images}：上传图片，返回包含宽高的 UploadResultDTO</li>
 ...
 * <ul>
 *   <li>有 {@code Range} 请求头：转发到 {@link ImageServeService#servePartialImage}，返回 206。</li>
 *   <li>无 {@code Range} 请求头：转发到 {@link ImageServeService#serveFullImage}，返回 200。</li>
 * </ul>
 * </p>
 */
@Slf4j
...
public class ImageAssetController {
     ...
     *
        ...
        * 上传成功后立即返回宽高数据，供客户端渲染骨架屏占位，避免布局跳动。</p>
     *
     ...
     *
        ...
        * 有 Range 返回 206 Partial Content；无 Range 返回 200 OK。</p>
     *
        * <p>注意：该接口不经过 {@link com.doublez.pocketmind.common.web.ApiResponseAdvice}
        * 的统一包装，因为响应 body 是二进制资源流，不适合 JSON 封装。</p>
     *
        * @param assetsUuid      路径中的附件 UUID
        * @param requestHeaders  来自请求的 HttpHeaders（含 Range 信息）
        * @return 200 全量或 206 分段响应（ResponseEntity 绕过统一包装）
     */
